...
final class GapWorker implements Runnable {
    ...
    ArrayList<RecyclerView> mRecyclerViewPluses = new ArrayList<>();
    long mPostTimeNs;
    ...
    public void add(RecyclerView recyclerView) {
        if (RecyclerView.DEBUG && mRecyclerViewPluses.contains(recyclerView)) {
            throw new IllegalStateException("RecyclerView already present in worker list!");
        }
        mRecyclerViewPluses.add(recyclerView);
    }

    public void remove(RecyclerView recyclerView) {
        boolean removeSuccess = mRecyclerViewPluses.remove(recyclerView);
    ...
    private void buildTaskList() {
        ...
        final int viewCount = mRecyclerViewPluses.size();
        int totalTaskCount = 0;
        for (int i = 0; i < viewCount; i++) {
            RecyclerView view = mRecyclerViewPluses.get(i);
    ...
    public void run() {
        try {
            ...
            if (mRecyclerViewPluses.isEmpty()) {
                // abort - no work to do
                return;
            }
            ...
            final int size = mRecyclerViewPluses.size();
